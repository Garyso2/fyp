...
class WiFiManager:
    """專責處理 Raspberry Pi 的 WiFi 相關系統指令"""
    @staticmethod
    def rescan():
    ...
    def get_nearby_ssids() -> List[str]:
        ...
        return ssids[:10]  # 只回傳前 10 個防止藍牙塞車
...
class VisualGuardServer:
    ...
    def notify_client(self, message: str):
            ...
            if self.server:
                try:
                    ...
                    # 🛡️ 核心防呆機制：如果傳送失敗，代表手機已經強行斷開！
                    print(f"⚠️ 發送失敗 (手機可能已斷線): {e}")
                    print("🛑 自動觸發煞車，重置狀態等待新連線...")
                    self.abort_tasks = True
                    self.is_connected_to_wifi = False

    def _wifi_scan_loop(self):
        """背景循環掃描 WiFi 任務 (加入煞車機制)"""
        print("🔄 [Thread] 啟動持續 WiFi 掃描...")
        # 條件加入 not self.abort_tasks
        while not self.is_connected_to_wifi and not self.abort_tasks:
            WiFiManager.rescan()
            
            if self.abort_tasks: break # 掃描完檢查有無被叫停
    ...
    def _connection_task(self, ssid, password):
        """處理連線邏輯與 3 分鐘 Timeout"""
        print(f"⏳ [Thread] 開始嘗試連線 {ssid}...")
        start_time = time.time()
        
        # 建立一個 thread 跑連線，方便我們在外面計時
    ...
    def handle_write(self, char: BlessGATTCharacteristic, value: bytes, **kwargs):
            ...
            if command == "CANCEL_SETUP":
                print("🛑 收到取消指令！終止所有掃描並重置狀態，等待新連線...")
                self.abort_tasks = True
                self.is_connected_to_wifi = False

            elif command == "SCAN_WIFI":
                self.abort_tasks = False # 放開煞車掣
                self.is_connected_to_wifi = False
    ...
    async def setup(self):
        ...
        await self.server.add_new_characteristic(
            SERVICE_UUID, CHAR_UUID, char_flags, b"Ready", permissions
        )
